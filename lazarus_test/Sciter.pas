...
uses
  ...
  Contnrs, Variants, Math, Graphics, LCLType, LMessages;
...
type
  ...
  public
    ...
    procedure GC;
    function GetElementByHandle(AHandle: Integer): IElement;
    function GetMinHeight(AWidth: Integer): Integer;
...
begin
  ...
  begin
    h := HWINDOW(Self.Handle);

    API^.SciterSetCallback(h, LPSciterHostCallback(@HostCallback), Self);

    SR := API^.SciterWindowAttachEventHandler(h, LPELEMENT_EVENT_PROC(@_SciterViewEventProc), Self, UINT(HANDLE_ALL));
    if SR <> SCDOM_OK then
...
function TSciter.GetElementByHandle(AHandle: Integer): IElement;
begin
  Result := ElementFactory(Self, HELEMENT(AHandle));
end;
...
function TSciter.GetMinHeight(AWidth: Integer): Integer;
begin
  Result := API^.SciterGetMinHeight(Handle, AWidth);
end;
...
var
  pStr: PWideChar;
begin
  pStr := PWideChar(SysAllocStringLen(PWideChar(Url), 1024 * 10));
...
constructor TElementCollection.Create(ASciter: TSciter);
begin
  FSciter := ASciter;
...
function TSciterEventMapRule.QueryInterface(constref IID: TGUID;
...
function TSciterEventMapRule._AddRef: Integer; stdcall;
begin
  Result := S_OK;
end;

function TSciterEventMapRule._Release: Integer; stdcall;
begin
  Result := S_OK;
end;
